...
// Storage type for K-quant (Q5_K/Q6_K) weights in repack buffers (see ggml_hexagon_repack_storage_type).
...
static void repack_k_quant_tiled(ggml_tensor * t, const void * data, size_t size) {
    ...
    switch (stype) {
        case GGML_TYPE_Q8_0:
            repack_q8_0_tiled(t, tmp.data(), tmp.size());
            break;
        case GGML_TYPE_IQ4_NL:
            // IQ4_NL has identical block layout to Q4_0 (ggml_half d + uint8_t qs[16])
            repack_q4_0_tiled(t, tmp.data(), tmp.size());
            break;
        default:
...
static void repack_tiled_k_quant(void * data, const ggml_tensor * t, size_t size) {
    ...
    switch (stype) {
        case GGML_TYPE_Q8_0:
            repack_tiled_q8_0(tmp.data(), t, tmp.size());
            break;
        case GGML_TYPE_IQ4_NL:
            repack_tiled_q4_0(tmp.data(), t, tmp.size());
            break;
        default:
...
struct ggml_hexagon_opbatch {
    ...
    bool same_shape(const htp_tensor * h, const ggml_tensor * t) const {
        ...
        int64_t ne0 = t->ne[0];
        int64_t ne1 = t->ne[1];
        const bool is_repack = ggml_backend_buffer_is_hexagon_repack(t->buffer) && ggml_hexagon_is_repack_type(eff_type);
        if (is_repack) {
...
static bool ggml_hexagon_supported_mul_mat(const struct ggml_hexagon_session * sess, const struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_MXFP4:
            ...
            // Safety bound on the number of output rows. The matmul kernel streams weights from DDR
            // row-by-row (VTCM footprint is independent of ne[1]), so large output dims (e.g. the
            // lm-head vocab, 262144 rows) are supported.
            if (src0->ne[1] > (1 << 21)) {
                return false;
            }
            ...
            } else if (src0->buffer && !ggml_backend_buffer_is_hexagon_repack(src0->buffer)) {
                // Other quantized weights must also be repacked.
                return false;
            }
            break;
