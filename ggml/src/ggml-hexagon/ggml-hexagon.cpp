...
static enum ggml_status ggml_backend_hexagon_buffer_init_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor) {
    ...
    if (ggml_hexagon_is_repack_type(tensor->type)) {
        ...
        if (sess->needs_repack.count(tensor)) {
            extra->flags |= GGML_HEXAGON_TENSOR_BF16F16;
            sess->needs_repack.erase(tensor);
        }
    }
...
// BF16 weights are converted to F16 in place so the DSP can use the existing F16 matmul path.
// Both types are 2 bytes/element, so byte offsets map 1:1 and the tensor buffer is reused.
static void hexagon_convert_bf16_to_f16(ggml_fp16_t * dst, const ggml_bf16_t * src, size_t n) {
    for (size_t i = 0; i < n; i++) {
        dst[i] = GGML_FP32_TO_FP16(GGML_BF16_TO_FP32(src[i]));
    }
}

static void hexagon_convert_f16_to_bf16(ggml_bf16_t * dst, const ggml_fp16_t * src, size_t n) {
    for (size_t i = 0; i < n; i++) {
...
static void ggml_backend_hexagon_buffer_set_tensor(ggml_backend_buffer_t buffer,
    ...
    if (extra->flags & GGML_HEXAGON_TENSOR_BF16F16) {
        hexagon_convert_bf16_to_f16((ggml_fp16_t *) ((char *) tensor->data + offset),
                                    (const ggml_bf16_t *) data, size / sizeof(ggml_bf16_t));
        return;
    }
...
static void ggml_backend_hexagon_buffer_get_tensor(ggml_backend_buffer_t buffer,
    ...
    if (extra->flags & GGML_HEXAGON_TENSOR_BF16F16) {
        ...
        hexagon_convert_f16_to_bf16((ggml_bf16_t *) data,
                                    (const ggml_fp16_t *) ((const char *) tensor->data + offset),
...
static void ggml_backend_hexagon_buffer_set_tensor_2d(ggml_backend_buffer_t buffer,
    ...
    if (extra->flags & GGML_HEXAGON_TENSOR_BF16F16) {
        for (size_t i = 0; i < n_copies; i++) {
            hexagon_convert_bf16_to_f16((ggml_fp16_t *) ((uint8_t *) tensor->data + offset + i * stride_tensor),
                                        (const ggml_bf16_t *) ((const uint8_t *) data + i * stride_data),
                                        size / sizeof(ggml_bf16_t));
        }
        return;
    }
...
static void ggml_backend_hexagon_buffer_get_tensor_2d(ggml_backend_buffer_t buffer,
    ...
    if (extra->flags & GGML_HEXAGON_TENSOR_BF16F16) {
        for (size_t i = 0; i < n_copies; i++) {
            hexagon_convert_f16_to_bf16((ggml_bf16_t *) ((uint8_t *) data + i * stride_data),
                                        (const ggml_fp16_t *) ((const uint8_t *) tensor->data + offset + i * stride_tensor),
                                        size / sizeof(ggml_bf16_t));
        }
        return;
    }
...
// BF16 weights are stored as F16 (converted in set_tensor); the DSP sees them as F16.
...
static bool ggml_hexagon_supported_mul_mat(const struct ggml_hexagon_session * sess, const struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_BF16:
            ...
            // Mirror the F16 branch constraints.
            if (src0->nb[1] < src0->nb[0]) {
                return false;
            }
            if (src1->ne[2] < src0->ne[2] || src1->ne[3] < src0->ne[3]) {
            ...
            if (!src0->buffer) {
                // not allocated yet: register so init_tensor/set_tensor convert it to F16
                sess->needs_repack.insert(src0);
            } else {
                // only claim BF16 weights that set_tensor converted to F16
                auto extra = (const ggml_hexagon_tensor_extra *) src0->extra;
                if (!ggml_backend_buffer_is_hexagon(src0->buffer) || !extra ||
                    !(extra->flags & GGML_HEXAGON_TENSOR_BF16F16)) {
                    return false;
                }
            }
