...
// Q5_K native 5-bit tile (32 rows x 32 k), same nibble plane as Q6_K. A tile covers exactly one
// Q5_K sub-block, so it holds a single scale/min pair per row.
//   vectors 0..3: low nibbles, vector i holds group 2i (low nibble) and group 2i+1 (high nibble)
...
// --- Weight Type Helpers ---
// Types stored in the tiled layout produced by the host repack.
...
static inline size_t htp_mm_act_tiled_row_size(int weight_type, uint32_t ne) {
    return htp_mm_weight_has_min(weight_type) ? htp_mm_q8_1_tiled_row_size(ne) : htp_mm_q8_0_tiled_row_size(ne);
}

static inline size_t htp_mm_act_flat_row_size(int weight_type, uint32_t ne) {
    return htp_mm_weight_has_min(weight_type) ? htp_mm_q8_1_flat_row_size(ne) : htp_mm_q8_0_flat_row_size(ne);
}
...
static inline void htp_mm_hvx_vtcm_layout_build(
    ...
    if (is_fused_nx) {
        ...
        size_t flat_act_row_size  = htp_mm_act_flat_row_size(wtype, ne10);
        size_t tiled_act_row_size = htp_mm_act_tiled_row_size(wtype, ne10);
        ...
        const size_t src1_row_size_tiled = htp_mm_act_tiled_row_size(wtype, ne10);
        ...
        switch (kernel_type) {
            ...
            case HTP_MM_KERNEL_HVX_QUANT_ROW: {
                size_t q_src1_row_size = htp_mm_act_tiled_row_size(wtype, ne10);
            ...
            case HTP_MM_KERNEL_HVX_QUANT_ROW_FLAT: {
                size_t q_src1_row_size = htp_mm_act_flat_row_size(wtype, ne10);
