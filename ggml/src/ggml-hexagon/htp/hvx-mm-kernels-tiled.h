...
// Low nibble plane, shared by the K-quant tiles: k-group g sits in vector g>>1, low nibble for even g, high for odd
static inline HVX_Vector unpack_kq_lo_nibbles(const HVX_Vector * restrict vptr, int g, HVX_Vector mask_0f) {
    return (g & 1) ? Q6_Vub_vlsr_VubR(vptr[g >> 1], 4) : Q6_V_vand_VV(vptr[g >> 1], mask_0f);
}
...
static inline HVX_Vector unpack_q6_k_group(const HVX_Vector * restrict vptr, int g, HVX_Vector mask_0f, HVX_Vector mask_03, HVX_Vector i32) {
    HVX_Vector v_lo = unpack_kq_lo_nibbles(vptr, g, mask_0f);
...
// Q5_K weights are stored unsigned (0..31), see HTP_MM_WEIGHT_TILE_SIZE_Q5_K. The min is applied by the
// affine epilogue, so the quants are left as-is.
static inline HVX_Vector unpack_q5_k_group(const HVX_Vector * restrict vptr, int g, HVX_Vector mask_0f, HVX_Vector mask_01) {
    HVX_Vector v_lo = unpack_kq_lo_nibbles(vptr, g, mask_0f);
    HVX_Vector v_hi = g ? Q6_Vub_vlsr_VubR(vptr[4], g) : vptr[4];
    return Q6_V_vor_VV(v_lo, Q6_Vw_vasl_VwR(Q6_V_vand_VV(v_hi, mask_01), 4));
}

// a Q5_K tile holds one scale per row, so the 32 k accumulate into a single sum
static inline HVX_Vector accum_q5_k_32x1(
    const HVX_Vector * restrict vptr,
    const HVX_Vector * restrict v_act
) {
    HVX_Vector v_sum0 = Q6_V_vzero();
    HVX_Vector v_sum1 = Q6_V_vzero();
    HVX_Vector mask_0f = Q6_Vb_vsplat_R(0x0F);
    HVX_Vector mask_01 = Q6_Vb_vsplat_R(0x01);

    #pragma unroll
    for (int g = 0; g < 8; g += 2) {
        HVX_Vector v_W0 = unpack_q5_k_group(vptr, g + 0, mask_0f, mask_01);
        HVX_Vector v_W1 = unpack_q5_k_group(vptr, g + 1, mask_0f, mask_01);
        v_sum0 = Q6_Vw_vrmpyacc_VwVbVb(v_sum0, v_W0, v_act[g + 0]);
        v_sum1 = Q6_Vw_vrmpyacc_VwVbVb(v_sum1, v_W1, v_act[g + 1]);
    }

    return Q6_Vw_vadd_VwVw(v_sum0, v_sum1);
}

static inline HVX_VectorPair accum_q5_k_32x2(
    const HVX_Vector * restrict vptr,
    const HVX_Vector * restrict v_act0,
    const HVX_Vector * restrict v_act1
) {
    ...
    #pragma unroll
    for (int g = 0; g < 8; g++) {
        HVX_Vector v_W = unpack_q5_k_group(vptr, g, mask_0f, mask_01);
        v_sum0 = Q6_Vw_vrmpyacc_VwVbVb(v_sum0, v_W, v_act0[g]);
        v_sum1 = Q6_Vw_vrmpyacc_VwVbVb(v_sum1, v_W, v_act1[g]);
    }
...
// a Q4_K tile is a Q5_K tile without the high-bit plane, see HTP_MM_WEIGHT_TILE_SIZE_Q4_K
static inline HVX_Vector accum_q4_k_32x1(
    const HVX_Vector * restrict vptr,
    const HVX_Vector * restrict v_act
) {
    HVX_Vector v_sum0 = Q6_V_vzero();
    ...
    #pragma unroll
    for (int g = 0; g < 8; g += 2) {
        HVX_Vector v_W0 = unpack_kq_lo_nibbles(vptr, g + 0, mask_0f);
        HVX_Vector v_W1 = unpack_kq_lo_nibbles(vptr, g + 1, mask_0f);
        v_sum0 = Q6_Vw_vrmpyacc_VwVbVb(v_sum0, v_W0, v_act[g + 0]);
        v_sum1 = Q6_Vw_vrmpyacc_VwVbVb(v_sum1, v_W1, v_act[g + 1]);
    }
...
static inline HVX_VectorPair accum_q4_k_32x2(
    const HVX_Vector * restrict vptr,
    const HVX_Vector * restrict v_act0,
    const HVX_Vector * restrict v_act1
) {
    ...
    #pragma unroll
    for (int g = 0; g < 8; g++) {
        HVX_Vector v_W = unpack_kq_lo_nibbles(vptr, g, mask_0f);
        v_sum0 = Q6_Vw_vrmpyacc_VwVbVb(v_sum0, v_W, v_act0[g]);
        v_sum1 = Q6_Vw_vrmpyacc_VwVbVb(v_sum1, v_W, v_act1[g]);
    }
...
// Epilogue of the affine types (w = d * q + m): v_scale/v_offset hold the per-row d and m of the tile,
// v_scale_a the activation scale and v_sum_a the scaled sum of the activation quants.
static inline HVX_Vector scale_dm_32x1(HVX_Vector v_sum, HVX_Vector v_scale, HVX_Vector v_offset,
                                       HVX_Vector v_scale_a, HVX_Vector v_sum_a) {
    ...
    HVX_Vector v_scaled_dot  = hvx_vec_mul_f32_f32(Q6_Vsf_equals_Vw(v_sum), v_scale_comb);
    return hvx_vec_add_f32_f32(v_scaled_dot, v_offset_comb);
}
...
static void tiled_vec_dot_q4_1_32x1(const uint32_t n, float * restrict s, const void * restrict vx, const void * restrict vy, uint32_t valid_rows, const float * restrict sz) {
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        HVX_Vector v_sum_scaled = scale_dm_32x1(v_sum, Q6_V_lo_W(p_deal), Q6_V_hi_W(p_deal), v_act[8], v_act[9]);
...
static void tiled_vec_dot_q4_1_32x2(const uint32_t n, float * restrict s0, float * restrict s1, const void * restrict vx, const void * restrict vy0, const void * restrict vy1, uint32_t valid_rows, const float * restrict sz0, const float * restrict sz1) {
    ...
    for (; kt + 1 < n_k_tiles; kt += 2) {
        ...
        HVX_VectorPair p_deal0 = Q6_W_vdeal_VVR(vptr0[4], vptr0[4], -2);
        HVX_Vector v_scale0 = Q6_V_lo_W(p_deal0);
        HVX_Vector v_offset0 = Q6_V_hi_W(p_deal0);

        HVX_VectorPair p_deal1 = Q6_W_vdeal_VVR(vptr1[4], vptr1[4], -2);
        HVX_Vector v_scale1 = Q6_V_lo_W(p_deal1);
        HVX_Vector v_offset1 = Q6_V_hi_W(p_deal1);

        HVX_Vector v_sum_scaled_c0_0 = scale_dm_32x1(Q6_V_lo_W(v_sums0), v_scale0, v_offset0, v_act0_0[8], v_act0_0[9]);
        HVX_Vector v_sum_scaled_c1_0 = scale_dm_32x1(Q6_V_hi_W(v_sums0), v_scale0, v_offset0, v_act1_0[8], v_act1_0[9]);
        HVX_Vector v_sum_scaled_c0_1 = scale_dm_32x1(Q6_V_lo_W(v_sums1), v_scale1, v_offset1, v_act0_1[8], v_act0_1[9]);
        HVX_Vector v_sum_scaled_c1_1 = scale_dm_32x1(Q6_V_hi_W(v_sums1), v_scale1, v_offset1, v_act1_1[8], v_act1_1[9]);
    ...
    for (; kt < n_k_tiles; kt++) {
        ...
        HVX_Vector v_sum_scaled_c0 = scale_dm_32x1(Q6_V_lo_W(v_sums), v_scale, v_offset, v_act0[8], v_act0[9]);
        HVX_Vector v_sum_scaled_c1 = scale_dm_32x1(Q6_V_hi_W(v_sums), v_scale, v_offset, v_act1[8], v_act1[9]);
...
static void tiled_vec_dot_q5_k_32x1(const uint32_t n, float * restrict s, const void * restrict vx, const void * restrict vy, uint32_t valid_rows, const float * restrict sz) {
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        HVX_Vector v_sum = accum_q5_k_32x1(vptr, v_act);

        HVX_VectorPair p_deal = Q6_W_vdeal_VVR(vptr[5], vptr[5], -2);
        HVX_Vector v_sum_scaled = scale_dm_32x1(v_sum, Q6_V_lo_W(p_deal), Q6_V_hi_W(p_deal), v_act[8], v_act[9]);
...
static void tiled_vec_dot_q5_k_32x2(const uint32_t n, float * restrict s0, float * restrict s1, const void * restrict vx, const void * restrict vy0, const void * restrict vy1, uint32_t valid_rows, const float * restrict sz0, const float * restrict sz1) {
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        HVX_VectorPair v_sums = accum_q5_k_32x2(vptr, v_act0, v_act1);

        HVX_VectorPair p_deal = Q6_W_vdeal_VVR(vptr[5], vptr[5], -2);
        HVX_Vector v_scale = Q6_V_lo_W(p_deal);
        HVX_Vector v_offset = Q6_V_hi_W(p_deal);

        v_sum_float_c0 = hvx_vec_add_f32_f32(v_sum_float_c0,
            scale_dm_32x1(Q6_V_lo_W(v_sums), v_scale, v_offset, v_act0[8], v_act0[9]));
        v_sum_float_c1 = hvx_vec_add_f32_f32(v_sum_float_c1,
            scale_dm_32x1(Q6_V_hi_W(v_sums), v_scale, v_offset, v_act1[8], v_act1[9]));
    }
...
static void tiled_vec_dot_q4_k_32x1(const uint32_t n, float * restrict s, const void * restrict vx, const void * restrict vy, uint32_t valid_rows, const float * restrict sz) {
    const uint8_t * restrict tile_ptr = vx;
    const uint8_t * restrict y_q = vy;
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        HVX_Vector v_sum = accum_q4_k_32x1(vptr, v_act);
...
static void tiled_vec_dot_q4_k_32x2(const uint32_t n, float * restrict s0, float * restrict s1, const void * restrict vx, const void * restrict vy0, const void * restrict vy1, uint32_t valid_rows, const float * restrict sz0, const float * restrict sz1) {
    const uint8_t * restrict tile_ptr = vx;
    const uint8_t * restrict y0_q = vy0;
    const uint8_t * restrict y1_q = vy1;
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        HVX_VectorPair v_sums = accum_q4_k_32x2(vptr, v_act0, v_act1);
