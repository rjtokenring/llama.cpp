...
MATMUL_2D_REPACKED_IMPL(q4_k,       640,  tiled_vec_dot_q4_k_32x2,  tiled_vec_dot_q4_k_32x1)
...
MATMUL_2D_REPACKED_IMPL(q4_k_flat,  640,  flat_vec_dot_q4_k_32x2,   flat_vec_dot_q4_k_32x1)
...
MATVEC_2D_REPACKED_IMPL(q4_k,       640,  tiled_vec_dot_q4_k_32x1)
...
MATVEC_2D_REPACKED_IMPL(q4_k_flat,  640,  flat_vec_dot_q4_k_32x1)
...
static int hvx_mm_init_vec_dot(struct htp_mm_context * mmctx, enum htp_data_type type) {
    switch (type) {
        ...
        case HTP_TYPE_Q8_0:
            ...
            return 0;
        case HTP_TYPE_Q4_K:
            mmctx->type         = "q4_k_tiled-f32";
            mmctx->vec_dot_32x1 = tiled_vec_dot_q4_k_32x1;
            return 0;
        case HTP_TYPE_Q5_K:
...
static int hvx_mm_matmul(struct htp_ops_context * octx) {
    ...
    if (src1_nrows > 1) {
        if (is_repacked) {
            switch (src0->type) {
                ...
                case HTP_TYPE_Q4_K:   matmul_job_func = hvx_mm_2d_repacked_q4_k;   break;
        ...
        if (is_repacked) {
            switch (src0->type) {
                ...
                case HTP_TYPE_Q4_K:   matmul_job_func = hvx_mv_2d_repacked_q4_k;   break;
    ...
    switch (kparams->kernel_type) {
        ...
        case HTP_MM_KERNEL_HVX_QUANT_ROW_FLAT: {
            ...
            src1_row_size = htp_mm_act_flat_row_size(src0->type, ne10);
            ...
            if (src1_nrows > 1) {
                switch (src0->type) {
                    ...
                    case HTP_TYPE_Q4_K:   matmul_job_func = hvx_mm_2d_repacked_q4_k_flat;   break;
                ...
                switch (src0->type) {
                    ...
                    case HTP_TYPE_Q4_K:   matmul_job_func = hvx_mv_2d_repacked_q4_k_flat;   break;
        ...
        default:
            ...
            src1_row_size = htp_mm_act_tiled_row_size(src0->type, ne10);
            break;
    }
...
DEQUANTIZE_WORKER_LOOP_IMPL(q4_k)
...
static int hmx_mm_2d_f32(struct htp_context *ctx,
    ...
    switch (weight_type) {
        ...
        case HTP_TYPE_Q4_K:   dequant_worker_fn = dequantize_tiled_worker_loop_q4_k; break;
...
static int hvx_mm_matmul_id(
    ...
    size_t src1_row_size  = htp_mm_act_tiled_row_size(src0->type, ne10);
...
static int hvx_mm_matmul_id_nx(
    ...
    size_t src1_row_size = htp_mm_act_tiled_row_size(src0->type, act->ne[0]);
...
int op_matmul_nx(struct htp_ops_context * octx) {
    ...
    if (kparams->kernel_type == HTP_MM_KERNEL_HVX_QUANT_ROW_FLAT) {
        src1_row_size = htp_mm_act_flat_row_size(src0->type, act->ne[0]);
    } else {
        src1_row_size = htp_mm_act_tiled_row_size(src0->type, act->ne[0]);
    }
