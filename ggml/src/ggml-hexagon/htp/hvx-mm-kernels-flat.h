...
// Flat activations store one int8 per k, but vrmpy needs each group of 4 k replicated across the 32 rows
// of a weight tile. Build the 8 replicated group vectors of one 32-k block.
...
static inline void flat_replicate_act_32(HVX_Vector v_act_raw, HVX_Vector v_act_rep[8]) {
    HVX_Vector v_repl_ctrl = * (const HVX_Vector *) flat_act_repl_ctrl;
    #pragma unroll
    for (int g = 0; g < 8; g++) {
        v_act_rep[g] = Q6_V_vdelta_VV(g ? Q6_V_vror_VR(v_act_raw, g * 4) : v_act_raw, v_repl_ctrl);
    }
}
...
static void flat_vec_dot_q4_0_32x1(const uint32_t n, float * restrict s, const void * restrict vx, const void * restrict vy, uint32_t valid_rows, const float * restrict sz) {
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        flat_replicate_act_32(v_act_raw, v_act_rep);
...
static void flat_vec_dot_q4_0_32x2(const uint32_t n, float * restrict s0, float * restrict s1, const void * restrict vx, const void * restrict vy0, const void * restrict vy1, uint32_t valid_rows, const float * restrict sz0, const float * restrict sz1) {
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        flat_replicate_act_32(v_act0_raw, v_act0_rep);

        HVX_Vector v_act1_rep[8];
        flat_replicate_act_32(v_act1_raw, v_act1_rep);
...
static void flat_vec_dot_q4_1_32x1(const uint32_t n, float * restrict s, const void * restrict vx, const void * restrict vy, uint32_t valid_rows, const float * restrict sz) {
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        HVX_Vector v_sum_scaled = scale_dm_32x1(v_sum, Q6_V_lo_W(p_deal), Q6_V_hi_W(p_deal), v_scale_a, v_sum_a);
...
static void flat_vec_dot_q4_1_32x2(const uint32_t n, float * restrict s0, float * restrict s1, const void * restrict vx, const void * restrict vy0, const void * restrict vy1, uint32_t valid_rows, const float * restrict sz0, const float * restrict sz1) {
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        HVX_Vector v_sum_scaled_c0 = scale_dm_32x1(Q6_V_lo_W(v_sums), v_scale, v_offset, v_scale_a0, v_sum_a0);
        HVX_Vector v_sum_scaled_c1 = scale_dm_32x1(Q6_V_hi_W(v_sums), v_scale, v_offset, v_scale_a1, v_sum_a1);
...
static void flat_vec_dot_q5_k_32x2(const uint32_t n, float * restrict s0, float * restrict s1, const void * restrict vx, const void * restrict vy0, const void * restrict vy1, uint32_t valid_rows, const float * restrict sz0, const float * restrict sz1) {
    const uint8_t * restrict tile_ptr = vx;
    const uint8_t * restrict y0_q = vy0;
    const uint8_t * restrict y1_q = vy1;
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        v_sum_float_c0 = hvx_vec_add_f32_f32(v_sum_float_c0,
            scale_dm_32x1(Q6_V_lo_W(v_sums), v_scale, v_offset, v_scale_a0, v_sum_a0));
        v_sum_float_c1 = hvx_vec_add_f32_f32(v_sum_float_c1,
            scale_dm_32x1(Q6_V_hi_W(v_sums), v_scale, v_offset, v_scale_a1, v_sum_a1));
    }
...
static void flat_vec_dot_q4_k_32x1(const uint32_t n, float * restrict s, const void * restrict vx, const void * restrict vy, uint32_t valid_rows, const float * restrict sz) {
    const uint8_t * restrict tile_ptr = vx;
    const uint8_t * restrict y_q = vy;
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        HVX_Vector v_sum = accum_q4_k_32x1(vptr, v_act_rep);
...
static void flat_vec_dot_q4_k_32x2(const uint32_t n, float * restrict s0, float * restrict s1, const void * restrict vx, const void * restrict vy0, const void * restrict vy1, uint32_t valid_rows, const float * restrict sz0, const float * restrict sz1) {
    const uint8_t * restrict tile_ptr = vx;
    const uint8_t * restrict y0_q = vy0;
    const uint8_t * restrict y1_q = vy1;
    ...
    for (uint32_t kt = 0; kt < n_k_tiles; kt++) {
        ...
        HVX_VectorPair v_sums = accum_q4_k_32x2(vptr, v_act0_rep, v_act1_rep);
